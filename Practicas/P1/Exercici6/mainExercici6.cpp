...
void imprimirClients(vector<Client>& clients){
    for(vector<Client>::iterator it = clients.begin(); it !=clients.end(); ++it){
        cout << "nom " << it->getNom() << ", adreça " << it->getAdreca() << ", població " << it->getPoblacio() << ", telefon " << it->getTelefon() << ", DNI " << it->getDNI() << ", edat " << it->getEdat() << endl;
    }
}
...
void imprimirMascotes(vector<Client>& clients, vector<Mascota>& mascotes, string dni){
    ...
    for(vector<Client>::iterator it = clients.begin(); it !=clients.end(); ++it){
        if(it->getDNI() == dni) {
            ...
            for(vector<Mascota>::iterator itMascotes = mascotes.begin(); itMascotes !=mascotes.end(); ++itMascotes){
                cout << "El nom de la mascota es " << itMascotes->getNom() << ", el tipus es " << itMascotes->getTipus() << ", la raça es " << itMascotes->getRaca() << ", el genere es " 

...
class MinHeap {
    public:
        MinHeap(); // constructor
        MinHeap(const MinHeap<Clau,Valor>& orig); // constructor còpia MinHeap<Clau,Valor> (const MinHeap<Clau,Valor>& orig); // constructor còpia
        int tamany() const; // retorna el nombre d'elements que hi ha al heap
        bool esBuit() const; // retorna cert si és buit, fals en cas contrari
        void inserir(const Clau& clau, const Valor& valor);
        Clau minim(); // retorna la clau mínima
        Valor valorMinim() const; // retorna el valor de la clau mínima
...
template<class Clau, class Valor>
Valor MinHeap<Clau, Valor>::valorMinim() const{
    if(esBuit()){
        throw invalid_argument("El heap es buit ");
    }
...
Valor MinHeap<Clau, Valor>::cerca(const Clau& clau){
    ...
    throw std::runtime_error("No s'ha trobat la key");
}

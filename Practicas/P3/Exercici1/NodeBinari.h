...
void NodeBinari<Clau, Valor>::insertValue(const Valor & v){
    //primer comprobo si ha d'anar a la dreta o a la esquerra a partir del value
    if(v < value){
        if(hasLeft()){
            _left->insertValue(v);
        }else{
            _left = new NodeBinari<Clau, Valor>(getKey(), v);
            _left->setParent(this);
        }

    }else{ //com es mes gran va a la dreta
        if(hasRight()){ //si te dreta l'afegeixo, si no en te la creo
            _right->insertValue(v);
        }else{
            _right = new NodeBinari<Clau, Valor>(getKey(), v);
